...
public class Boj_15661 {
    ...
    static void dfs(int numberOfTeamMembers, int start, int depth) {
        ...
        for(int i = start; i < N; i++) {
            if(!visited[i]) {
                visited[i]=true;
                dfs(numberOfTeamMembers,i + 1,depth + 1);
                visited[i]=false;
            }
        }
    }
    ...
    static void twoTeamStatsRoof() {
        ...
        for(int i = 0; i < N; i++) {
            for(int j = i + 1; j < N; j++) { // 다음 사람은 i와 중복되지 않게 다음 사람부터 탐색
                if(visited[i] && visited[j]) { // startTeam팀
                    startTeam += inputDataArr[i][j];
                    startTeam += inputDataArr[j][i];
                }
                else if(!visited[i] && !visited[j]){ // linkTeam팀
